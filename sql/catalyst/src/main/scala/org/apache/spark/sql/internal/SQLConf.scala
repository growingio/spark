...
object SQLConf {
  ...
  val ADAPTIVE_EXECUTION_SKEWED_PARTITION_FACTOR =
    buildConf("spark.sql.adaptive.skewedPartitionFactor")
      .doc("A partition is considered as a skewed partition if its size " +
        "is larger than this factor multiple the median partition size and also larger than " +
        "spark.sql.adaptive.skewedPartitionSizeThreshold, or if its row count " +
        "is larger than this factor multiple the median row count and also larger than " +
